...
const Top = {
  ...
  async allTime(req, res) {
    const query = `SELECT id, rank, weight_start, weight_diff, first_name, last_name, avatar_src FROM users WHERE weight_diff IS NOT NULL AND rank!=0 ORDER BY (arr_last_item(weight_history)->>'weight')::real/(weight_start/100)`
